  ...
  return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="max-w-md w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          <div className="rounded-md shadow-sm -space-y-px">
            <textarea id="memory" required className="appearance-none rounded-none w-full px-3 py-2 border border-gray-300 text-gray-900 rounded-t-md" placeholder="Write your memory here..." rows={4} value={memory} onChange={(e) => setMemory(e.target.value)} />
            <input id="file" type="file" className="appearance-none rounded-none w-full px-3 py-2 border border-gray-300" onChange={(e) => setFile(e.target.files ? e.target.files[0] : null)} />
            <input id="unlockDate" type="date" required className="appearance-none rounded-none w-full px-3 py-2 border border-gray-300 rounded-b-md" value={unlockDate} onChange={(e) => setUnlockDate(e.target.value)} />
          </div>
        ...
        {success && <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-2 text-green-600">Time capsule created successfully!</motion.p>}
      </motion.div>
    </div>
  )
}
